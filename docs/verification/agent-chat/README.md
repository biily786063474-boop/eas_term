...
## CLI 自动更新设置（2026-09-07）

设置→更新增加 Codex / Claude Code 独立自动更新开关，默认关闭；显示当前版本、待重启版本、下载/错误状态。下载写独立版本目录，重启校验后才改变启动 PATH，当前进程和会话不切换；内置 harness 随应用更新。失败保留旧版，回退到上一托管版也在重启生效并关闭自动更新。原应用更新检查开关明确标为 Eas-Term，避免与 CLI 默认值混淆。

75 项更新生命周期、包完整性/解包、PATH、adapter/sessionState 相关测试通过；类型检查、构建、27 文件 CSS 平衡与 diff 检查通过。`scripts/verify-cli-updates.mjs` 用独立 userData/真实 Electron 和 preload IPC：鼠标打开设置和更新页、两个开关默认关闭、Codex 开/关写盘、30×18 控件无横向越界；重启后 pending 99.0.1 测试 CLI 变为 active，实际 agent.probe 执行了该托管二进制（通过独立调用记录断言）。证据 `cli-update-e2e.json` / `cli-update-settings.png`，截图已查看。测试 CLI 是明确的本地可执行 fixture，不是真实供应商版本。

官方 npm 元数据和原生包映射已联网核对。完整 Codex 原生包下载实测因当前网络速度在原五分钟限时处中断，临时包已清理，没有改系统安装；最终下载限时改为十分钟，界面提供超时与重试提示。未完成最新供应商原生包的完整网络下载验证；测试包覆盖 SHA512 失败、非官方来源拒绝、真实 tar 解包、资源保留与 --version/--help 校验。未在 Windows/Linux 真机验证，也不保证上游所有插件/模型接口兼容。
