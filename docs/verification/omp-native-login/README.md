...
## 2026-09-08 Google 项目缺失：真实失败定位与提示补充
测试实例原生日志（PID 23867）在 09:39:56.583 -07:00 报 configuration / google-gemini-cli：
账号要求 GOOGLE_CLOUD_PROJECT 或 GOOGLE_CLOUD_PROJECT_ID。渲染事件 09:39:56.587 failed，
09:39:59.864 才关闭弹窗，故该次不是关闭造成失败。浏览器成功页不等于最终凭证保存。
新增安全分类、中文配置提示和“已配置项目，重新登录”按钮文案。
定向解析/控制器/分类/React 渲染测试 64/64 通过；typecheck 通过。
尚未新增项目 ID 设置入口；尚未验证配置项目后的真实账号登录，不得标记 Google 登录已修复。
