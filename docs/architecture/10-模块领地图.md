...
执行清单后台插件（2026-09-25）：`resources/plugins/execution-plan/` 拥有 stdio MCP、`ui://` 面板与项目根 `.eas/execution-plans.json`（原子写、CAS、模型报告与用户验收分离）；`main/executionPlanAuthorization.ts`、`mcpBridge.ts`、`pluginHost.ts`、`agentChat/executionPlanTurns.ts` 共同守住受管会话租约/项目根/用户轮次，`executionPlanSnapshot.ts` 只读恢复摘要。`AgentChatView` 复用 `PluginPanel` 在对话内弹层，不建画布节点；PTY 不自动接入。改这些位置要同步 03/11/13 号图纸，不调整 `index.ts whenReady()` 注册顺序。
