...
- **`CanvasContextMenu.tsx` 是违章建筑**：被 agentChat / workspace(3 处) / terminal / gantt / files / git 依赖，但**不都在用组件本体** —— workspace/`ModeSwitch.tsx` 与 gantt 只拿 `useMenuAnchor` / `useDismiss`（gantt **刻意不复用组件本体**，文件头写了理由：`.canvas-ctxmenu` 的 z-index 比甘特浮层低），workspace/`projectMenu.ts` 只 import 类型。它的导出全是对外 API，动哪个都要全量 `grep -r CanvasContextMenu src/renderer`。**它本该在 `ui/`，实际长在 canvas 里** —— 挪进 `ui/` 是正确方向，但那是一次独立重构，别顺手做。
