...
2026-09-18 双目录构建：`scripts/plugin-registry-build.mjs`被build-plugin-registry调用，先暂存打包/清单+目录校验、全量检查版本包不可变性，再复制新包并分别rename原子更新registry-v2.json/registry.json。v1严格排除所有带requirements的包，v2保留requirements和unavailable。两个目录不是跨文件事务；默认仍仅收录已有两包，新增能力/32项不能由构建通过推定已发布。实际生产publish脚本尚待切换，本批不运行它、不改宿主默认URL。
