...
## 设置层级导航（2026-09-10）
workspace/SettingsPanel.tsx 保留偏好/审批/状态栏/快捷键处理器；settingsNavigation.ts 仅承载10页/3组及搜索关键词，无偏好副本。settingsHierarchy.css 仅通过 cset-settings 作用于设置弹层，复用 cset-box 的更新弹层材质不变。McpIndicator 的 eas:open-settings 目标改为 mcp；旧 ai 目标仍为AI对话。黑匣子UI移到perf，Footprint仍在privacy；权限档位沿用原setApproval/安全规则。
