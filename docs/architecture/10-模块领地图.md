...
`canvas/layout.ts` 的 `placeNodeInFrame` 通过纯函数 `compactPlacement.ts` 选址：已有内容包围盒内可用空位优先，其次最小化外接正方形边长、面积，按行序破同分。按真实宽高及 GAP 避让，直属子 Frame 也是障碍（折叠按标题高度）。不移动旧模块，不改变手动拖放的 placeNodeAtPoint/findFreePos，也不改变 tidyFrame 整体整理。测试 compactPlacement.test.ts；隔离应用四个 AI 新建模块形成 2×2 验证。
