...
| 分组 | 文件 | 责任域 / 坑 | 风险 |
|---|---|---|---|
| `src/main/agentChat/codexModels.ts` | Codex 的可用模型清单（问 `codex app-server` 的 `model/list`） | 🟢 | **别改成硬编码**：Codex 模型名带小版本（gpt-5.6-sol / terra / luna）且**每个账号能用的不一样**，写死等于隔三差五给用户一个选了就报错的选项（2026-09-06 用户报「无法下拉选择模型」）。探测起一个短命 `app-server` 进程、结果缓存；失败返回 undefined，工具栏退回没有下拉。<br>接线：`CliAdapter.probeModels` 可选钩子 → `session.ts` 会话起来后**不 await** 地调一次，拿到就广播 `capabilities` 事件更新下拉 |
