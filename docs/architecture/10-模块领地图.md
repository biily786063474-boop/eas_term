...
Codex 路由超时安全恢复（2026-09-23，开发分支）：`mcp/codex-task-recovery.mjs` 只认原生终止轮的精确 `workspace routing discovery timed out`，并要求无执行活动、无用量、无 active goal、无其他 active turn、未取消且未达到两次上限；`codex-task-error.mjs` 只输出固定类别和真实重试次数，绝不回显提供商错误正文。桥接分叉接线另见 03 号图纸。
