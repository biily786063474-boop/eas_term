...
### 2026-09-09 语音候选改动（尚未完整验收）
`features/voice/voiceTarget.ts` 集中选区拼接，ChatToolbar/AgentChatView 启动页/TerminalInput/Todo 详情接入；VoiceButton 点麦克风不夺焦点，切到其他编辑框先停止，尚非跨框连续听写。`main/voiceVad.ts`/`voiceVadWorker.ts` 在独立 Worker 等待 WASM runtime 初始化后做 Silero VAD；`voiceGate.ts` 限定前置与尾静音缓存。`stt.ts` 以 VAD 决定人声而非 RMS，保留原 Zipformer/SenseVoice。记录所有权、epoch、stop drain、防迟到写入；新增 `stt:error` 必须与 preload 同步。实测状态见 docs/verification/voice/README.md，不把假 ASR UI 回放说成真实麦克风验收。
