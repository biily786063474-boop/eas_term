...
发布工作流（2026-09-26）：`.github/workflows/build.yml` 的 tag Windows job 只把 EXE 上传到 **draft** GitHub Release；Mac 双架构包、Windows 同提交验收齐全后才人工公开。勿去掉 `draft: true`，否则 tag CI 会先把只有 Windows 单包的不完整 Release 暴露给用户。

Markdown 阅读强调（2026-09-26）：`features/editor/markdown.ts` 是文档预览、知识库、AI 回答共用的 HTML 渲染器；三者都带 `.md-view`，加粗 `<strong>` 的颜色统一由 `editor.css` 中 `.md-view strong` 和 `styles/base.css` 的亮暗 `--md-emphasis` 管，勿在单个阅读模块再写死白/黑或各自覆盖。

页面观察窗与汇报页联动（2026-09-25）：`canvas_publish_report` 是**语义明确的提交工具**，不是从三家 CLI 回复文本猜测报告。`mcpHandler.ts` 沿用项目路径校验、`probePaths` 存在性检查、提交后 `resolveFrame` 校验；`openArtifact` 对此工具不切到画板、不聚焦隐藏节点，普通 `canvas_open_html` 仍保留旧行为。跨平台路径与 URL 由 `shared/artifactPath.ts` 归一化（盘符、UNC、特殊字符）；`main/reportFileUrl.ts` 用平台原生 `fileURLToPath` 解码，**不能把 `URL.pathname` 直接当 Windows 路径**。`livePage/reportAssociation.ts` 只按 AI leaf＋Frame＋web node URL 关联，节点失效不显示；`livePageStore.ts` 在观察会话结束时清关联。`LivePagePanel.tsx` 的上下布局与局部最大化仅消费同 leaf 的已有观察会话，报告独立受限 webview 不进入 loopback-only `livePage` 浏览器。双层展示时 `CanvasFileNode` 暂停原 HTML guest，避免重复 Chromium 页面；报告 guest 创建前必须通过 `fs:validateReport` 无缓存 realpath 项目授权复核，失效即销毁，不能用终端链接的 `probePaths` 缓存当授权；主进程禁止该 guest 后续导航和弹窗，Esc 经限定 partition 转发给宿主。切换 leaf 时仅自动暂停捕帧并在返回时恢复，手动收起不自动恢复；其他沉浸层取得所有权后退出本视图最大化。画板模式沉浸层 portal 到 body，避免被画布 transform 限住；调试 BrowserWindow 原会话不复制。新增工具时 `mcp/workbench-tools.json`、schema 测试及受管指引须同步，不要改为扫描正文或常驻长提示词。

对话提问导航性能（2026-09-25）：`agentChat/QuestionNavigator.tsx` 会随画布几何变化更新 body portal 的轨道位置，但画布纯平移不改变提问在消息容器内的相对位置。`questionIndex.ts` 的 `QuestionTopMeasure` 缓存这一判据：仅滚动、缩放比例变化、正文 DOM 变化、异步媒体加载、字体加载或容器 resize 时重读每条提问的 rect。**不能停止几何观察器**，否则轨道在平移时脱节；也不能把正文变化当作纯平移，否则活动提问会错位。隔离应用 4 对话×60 提问、60 帧平移：提问 rect 读取 4560→120；滚动后活动提问仍更新。此优化不代表长运行内存或标题栏合成闪烁已解决，见 `docs/verification/performance/2026-09-25-root-cause.html`。

最大化隐藏画布的装饰动画（2026-09-25）：`.canvas-world.hidden-by-max` 只设 `visibility:hidden`，保留节点挂载与 webview/终端身份；但这**不会停止后代 CSS animation**。`FrameStart` 的 `data-offscreen` 由几何相交判据决定，最大化遮住 world 时仍可能保留「相交」，导致看不见的 `cframe-sweep` 继续转。`canvas.css` 对 `.canvas-world.hidden-by-max .cframe-start-ring::before` 单独暂停，不对整个 world 统一暂停，避免误停被子元素显式恢复可见的最大化 HTML 节点。隔离应用强制该边界：36 个运行光环→36 个暂停光环。

执行清单原生任务卡（2026-09-25）：`resources/plugins/execution-plan/` 仍是 `.eas/execution-plans.json` 唯一写入者（ownerKey、CAS、原子写；旧无 owner 计划只留历史）。模型仅见原五工具；`executionPlanOwner.ts` 从已保存的 canvas/project 校验稳定 `node:<id>`，临时分屏用 `session:<id>`；`executionPlanAuthorization.ts` 覆盖模型自报身份。`executionPlanCardIpc.ts` 经 `pluginHost.requestExecutionPlanCard` 的路径/启用/根重验调用私有卡片 RPC；`agentChat/session.ts` 负责确认停止与空闲收尾。`AgentChatView` 顶部挂 `PlanCard`，`PluginPanel` 仅作详情，不再在消息流重复入口。PTY 不自动接入。`index.ts` 只在原 `registerAgentChatHandlers()` 后追加卡片注册，不得重排既有顺序；改这些位置同步 03/11/13 号图纸。

Git 历史分页（2026-09-25）：`HistoryView` 首次读取 200 条，用户向下滚到列表底部附近时通过 `git:log(cwd, 50, skip)` 追加 50 条；只允许一笔增量请求在途，刷新/切项目使旧请求失效。`main/git.ts` 以 `--skip` 和 `-n` 在 Git 侧分页，preload 保持旧两参数调用兼容；轨道图仍对当前已加载的完整数组计算，勿分段单独计算。`HistoryView` 的 `computeGraphRows` hook 必须保留在非仓库提前返回之前。
...
Jev 凭证引导（2026-09-24）：`PluginConfigurationControls` 保存/测试时在原设置内复用 `VaultGate`，保留草稿并仅在成功保存后清空；`PluginPanel` 对 Jev 连接验证的锁定状态弹宿主解锁，不跨 35 秒面板 RPC 等候，解锁后需用户再次验证。`secrets.ts` 的“信任此设备”只存布尔选择，密钥仍由 safeStorage 加密；手动 `secrets:lock` 必须撤销信任及插件租约。Jev 插件专属凭证不出现在通用密钥分组，不能为了“看见”而复制明文。详见 [`2026-09-24-jev-vault-onboarding.md`](../superpowers/specs/2026-09-24-jev-vault-onboarding.md)。
...
| `features/livePage` | AI 页面开发观察窗的共享订阅、分屏抽屉和画板对话内预览 | 🟢 | 浏览器只在主进程运行；这里消费受限截图，`App` 常驻订阅、`PaneView` 按 leafId 归属显示。隐藏时保留会话，结束时清理。 |
...
**页面开发观察窗（2026-09-25）：** `livePagePolicy.ts` 限定 loopback 顶层 URL 与子资源、会话所有权和输入范围；`livePage.ts` 管独立 `BrowserWindow`、低频 `capturePage`、串行导航、崩溃清理及生命周期，`mcpBridge.ts` 将 `page_live_*` 直达主进程。`livePageWindowTag.ts` 标记观察窗，`island.mainWindow()` 和 `index.ts` 的 Dock activate 逻辑必须排除它，避免把截图/MCP IPC 送进网页窗口。`preload/index.ts` 是 UI 唯一控制入口，渲染层 `features/livePage` 只显示截图，不持有浏览器；分屏优先显示当前 AI leaf 的预览，离开可见视图暂停捕帧。工具清单增删需同步 `mcp/workbench-tools.json` 与 `workbenchSchema.test.ts`。
...
Frame 右键菜单（2026-09-25）：`stageMenu.ts` 将「新建」限制为 AI 对话/终端/浏览器，另设一级「组件」子菜单，选项继续从 `CANVAS_COMPONENTS` 注册表生成，项目要求和插入坐标不变。`CanvasContextMenu` 的子菜单箭头统一增强可见度；`quietDanger` 仅用于「删除 Frame」的中性外观，保留 `danger` 标记及原删除动作，其他危险项仍使用警示样式。`scripts/verify-frame-context-menu.mjs` 在隔离 Electron 的 Frame 右键位置核对暗/亮模式、菜单层级、箭头和组件清单。
...
2026-09-24 市场更新/卸载遇运行中插件：`pluginMarket.installCommit/uninstall` 经 `pluginHost.withPluginPackageMutation` 持有单插件 mutation fence，主窗口原生弹窗确认后只停止该插件，等待真实退出再改包；取消、宿主 lease 改变、超时都不写目标目录。不能让用户自行去运行中心找进程，也不能全局终止其他插件。`verify-plugin-hot-update.mjs` 用隔离真实应用 + 活跃 MCP 进程核验确认与更新；VM 边界测试核验取消。
...
## 2026-09-24 · AI 对话用量口径与本地审计

`Usage.inputIncludesCached` 是可选口径标记：Codex adapter 标记输入已包含缓存，旧历史由 ChatToolbar 按 CLI 兼容；Claude 原有的独立缓存口径不变。`chatStats` 不得无条件重复加缓存输入；已知 0% 与未知命中率要区分。`capabilityGuidance` 将目录只声明一次，文件名相对于该目录，安全说明及开关语义不变。`scripts/token-dashboard/` 只读本项目本机 Codex rollout，按 response_id 去重、turn_id 汇总；输出私有本地 `docs/token-dashboard.html`（已忽略），不把推算分词当作准确的隐藏提示或工具 Schema 用量。
### 2026-09-21 灵动岛只显示最终回答

`main/session.ts` 的解析缓存额外保存每轮 `finalAnswers`，`session:last` 仅取该轮最后一条纯文本 assistant 消息再做原有 260 字摘要；工具调用清空先前候选，混合 text/tool_use 不作为结果，thinking/sidechain 不进入结果。新用户轮次清空结果，绝不回退到过程文本。`session:exchange` 仍保留完整文字与工具标注，灵动岛审批内容与权限不变。回归：`src/main/session.test.mjs`；隔离应用 IPC→灵动岛截图：`docs/verification/island-final/`。此链路仍是 Claude transcript，不新增 Codex 记录解析。

### 2026-09-21 灵动岛模块／会话／轮次精确归属

- `features/status/islandResults.ts`：每次 AI 对话 attach 独立收集公共事件，只在 turn.done 发布最终文本；结果按内部 sessionId + leafId + lastDoneAt 绑定，turn.start/卸载清理，缓存有界。不读项目 Claude 日志来猜 AI 对话正文，Claude/Codex/omp 均走自身事件流。
- `useIslandFeed.ts`：分辨 agent/terminal。终端仅接受明确绑定的 Claude sessionId；异步请求代次及回填时的 tab/leaf/node/cwd/CLI/session/轮次必须一致。正文展示前再次核验身份；正在运行时不展示旧结果。终端最终回答 answeredAt 必须位于本轮开始与完成之间，缺少时间证据就不显示正文。通知 ID 使用 lastDoneAt，不再使用可能重复的耗时。
- `session:last` 通过严格模式 findJsonl 查找，缺失/无效/不存在的绑定返回 found:false，禁止回退到项目最新文件；session:index/exchange 的历史导航规则不变。SessionLast 新增可选 answeredAt（最终文本的 transcript timestamp）；preload 原样返回，无新增 IPC 或权限。
- `Island.tsx` 的按钮区分 AI 对话与终端；取不到精确结果时显示「未取得该模块本轮的最终回答」，不再宣称 Codex 没有记录。跳转仍由任务 ID 经 machine.locate/focusTerminal 精确落到 leaf。
- 验证：145 项相关测试通过，typecheck/build 通过；`scripts/verify-island-binding.mjs` 在隔离 Electron 的 9447 CDP / 9448 main inspector 回放公共事件，验证双 AI 模块、第二轮隔离、精确跳转以及双终端 transcript 绑定。不是远端真实模型调用。证据：`docs/verification/island-binding/`。运行脚本前须自行启动隔离 EAS_VERIFY 实例，不用于正式版。
...
### 2026-09-26 交互准入压力分级
`runtime/controller.ts` 仅将平台 `critical` 映射为严重压力；`warning` 不能阻止用户主动发送。后台 normal/eco 阈值和预算不变，未校准/缺失/过期采样仍只监测不拦交互。8/16/32GB 回归覆盖 warning 与 critical。
`runtime/scheduler.ts` 到期抛 `ResourceWaitTimeoutError`（code `RESOURCE_WAIT_TIMEOUT`），先到期后放行避免临界点重复投递。`startupFailure.ts` 只识别此类型为资源等待超时；普通 spawn/network timeout 保留真实启动错误，不许恢复全文正则匹配。

### 2026-09-26 未投递恢复与按需内存诊断
`agentChat/session.ts` 的 `runtimePendingMessage` 只活到 managed start callback 尝试投递前；排队失败/取消才发 `message.unsent`。spawn/网络错误或启动后的取消不得据此声称未发送。渲染层把该宿主事实存为 `Turn.unsentText`，随已有 history 存取；「恢复草稿」只回填，不触发 IPC 发送。`message.unsent` 先于 synthetic turn.done 暂停后续队列。首条用户记录在订阅缓冲事件前定位，避免错误说明排到提问前。
`runtime:monitor` 的可选 `includeProcesses=true`（仅可信 workbench）才调用 `processMetrics.ts`；5 秒缓存聚合 Electron 内存/CPU，无 PID、路径、消息或命令；默认监控不增加采样。不包含外部 CLI/插件，峰值和是各进程自报峰值之和，不等于同时峰值，不能用来宣称软件总内存下降。
首次启动还必须先经 `history.preserveBeforeStart` 保存原问题，再调用 start IPC；保存失败不启动。历史键与 seq 在开始动作时捕获，fallback 使用同一 seq，不依赖组件持续挂载或模型产生回答。仅记录用户提问，不把投递未知伪装成未发送。`recoveredDraft` 为一次性输入命令，消费后及新建对话时清空，防旧原文跨会话注入。

2026-09-26 续：按需 `runtimeMonitor(true)` 同时提供 `processTree.ts` 的主进程+活跃后代 RSS 数字聚合，CLI/插件后代不再漏计；Electron 原子集保留不可相加。5秒缓存、单在途、失败null；限定诊断、不可当准入或进程所有权证明，边界详见17号图纸。合成失败/停止结束事件带 interrupted/usageKnown，`status/islandResults.ts` 拒绝生成成功结果；真实正常结束不受影响。
