...
**页面开发观察窗（2026-09-25）：** `livePagePolicy.ts` 限定 loopback URL、会话所有权和输入范围；`livePage.ts` 管独立 `BrowserWindow`、低频 `capturePage`、页面操作及生命周期，`mcpBridge.ts` 将 `page_live_*` 直达主进程。`livePageWindowTag.ts` 标记观察窗，`island.mainWindow()` 和 `index.ts` 的 Dock activate 逻辑必须排除它，避免把截图/MCP IPC 送进网页窗口。`preload/index.ts` 是 UI 唯一控制入口，渲染层 `features/livePage` 只显示截图，不持有浏览器。工具清单增删需同步 `mcp/workbench-tools.json` 与 `workbenchSchema.test.ts`。
