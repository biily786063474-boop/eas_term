...
首轮模型入口：StartupModelPicker.tsx → preload.modelCatalog → session.resolveAdapterModels；首轮参数优先级在 startupParams.ts，AgentChatView 的两条启动路径都复用。

工具事件/MCP 容错：`agentChat/*Events.ts` → `shared/agentChat.ts` → renderer 的 reduce/history 必须同步维护可选 ExecKind；错误诊断位于 stderrReason/session 与 cliAuth/detect，详见 16 号图纸。
