...
| **Agent 生态** | `mcpBridge.ts` `agentRules.ts` `rulesRefresh.ts` `agentHook.ts` `agentSkill.ts` `agentInstall.ts` `agent.ts` `roles.ts` `rolesSchema.ts`（清洗 + v1→v2 迁移，electron-free）`cliContract*.ts` `plugins.ts` `mcpOptOut.ts` `statusline*.ts` | 对外部 CLI 的全部接口（`agent.ts` 是 CLI 探测：`agent:probe` / `agent:codexServers`，不碰 PTY） | 🔴 |
