...
## 2026-09-08 私发 Windows 诊断分支

`main/diagnostics/core.ts` 是诊断白名单与报告上限；`journal.ts` 有界结构化日志；`record.ts` 是零 Electron 事件 sink；`identity.ts` 只认独立诊断构建；`index.ts` 管主进程生命周期、原生确认、固定 userData 导出和回传；`transport.ts` HTTPS、不跟随重定向、必须核对回执。不要上传旧的自由文本日志。

只在成功取得单实例锁后 initializeDiagnostics，不重排 whenReady 的既有注册；新增 registerDiagnosticHandlers 在 registerDiagHandlers 后。session/omp/pty 的启动和退出只报白名单，不传参数或输出。设置沿用原有样式，仅诊断身份显示入口。诊断包独立 appId/name/userData，updater/telemetry 不出站。

本分支 `.github/workflows/build.yml` 为 workflow_dispatch-only，固定 windows-2022，权限 contents:read，只产出 Actions artifact，无正式 Release；不合并此分发配置到正式 main。`deploy/diagnostics/` 是独立接收服务源码，不改 eas-stats.py。服务部署需要备份 nginx、nginx -t、站点前后对照。

诊断运行验收脚本通过 inspector 在真实 `out/main/index.js` 入口断点注入原生按钮响应；不要在 Electron bootstrap realm 提前 Runtime.evaluate（Windows 首轮 CI 在这里超时）。按钮之外的上传/落盘/退出/重启均执行真实代码，失败不得跳过安装包交付门禁。

Windows诊断验收分两段共用同一userData/cwd：无主进程调试器执行真实ConPTY并等待回显，再通过入口断点代答原生确认；导出报告必须包含前段实际PTY事件。不得为调试器环境的ConPTY阻塞修改产品进程实现或跳过日志断言。
