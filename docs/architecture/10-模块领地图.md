...
2026-09-07 原型接入：StartupSetupCard 只派生当前 CLI 的安装/认证状态，AgentChatView 管理切换代次并复用 CliSetupPanel / OmpSetupPanel；QuestionNavigator 复用 MessageList 的真实提问 DOM，portal 几何兼容 Frame、最大化和分屏，保持吸顶零 padding-top。SemanticIcons / semanticIconKinds 为 FileTree、BranchBadge、工具执行和位置提供共享双色图标；ExecKind 同步 shared、三翻译器、reducer、history。约束与验证入口详见 16-AI对话公共协议。
