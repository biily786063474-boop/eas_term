...
`pluginConnections/oauthCallback.ts`：主进程内部授权尝试原语，随机127.0.0.1端口+随机路径，32字节state/verifier与S256 challenge；回调严格Host/GET/路径/重复参数/state/iss校验，单次消费，取消或默认180秒超时关闭所有所属socket。不打开浏览器、不交换/保存token；要求上游返回iss，未支持的上游失败关闭，不能声称已完成OAuth或账号连接。
