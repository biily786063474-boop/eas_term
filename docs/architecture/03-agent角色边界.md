...
### 2026-09-11 验收缺陷回归边界
- `secretRequest.ts` 持有弹窗，必须先于 `mcpBridge` 10 分钟上限超时（当前 9 分钟），清 pending + emit + reject；系统超时不能计作用户拒绝。旧计时器/异步保存不能消费下一次请求。
- `CodeView` 必须同步创建正文编辑器，语法高亮仅异步装进 Compartment。旧开发窗口跨 build 时 chunk hash 失效已实测导致空白，禁止恢复成等待语言模块成功才建 EditorView；失败降级纯文本，卸载后的加载结果不得写回。
- 产物刷新保护同时看 `editing` 和 `dirty`：刚进入编辑、保存后仍继续编辑都不能被重提交销毁；两者都为 false 才合并刷新。`onDirtyChange` 仍保留独立语义供灯箱防丢稿，不能偷偷改成 editing OR dirty。

### 2026-09-12 设计选型提示词边界
- 设计选型复用 `composerAddChip`，确认只附加所选系统，不发整个库，不直接调用模型、不写 PTY。用户仍从原输入框发送。
