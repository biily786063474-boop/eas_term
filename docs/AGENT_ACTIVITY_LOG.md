...
## 2026-09-23 看板右键快速移动
用户确认按项目状态划分的看板区，要求不再依赖跨屏拖动。独立分支 feat/board-quick-move-20260923，基线c32300fc；不混入未发布的边缘拖拽或时间线改动。新增看板卡片右键「移动到看板区」，画布菜单同步名称及幂等行为。隔离Electron真实右键验证：自定义列、点击归列、真实IPC持久化、当前列禁用、未分类、重载保留；正式应用未动。
