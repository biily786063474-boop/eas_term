...
5. **事件序列基线**（阶段 0.5 先修工具）：① 修 `verify-agent-chat-ui.mjs:219` 的锚点使其逐字等于 `main.tsx:23`（它现在跑不起来，14-验证与调试:175 已记）；② `verify-agent-chat.mjs` 加 `--cli=<claude|codex>` 与 interrupt / stop 两步，把 `events` dump 成 JSON 基线；③ 不花额度的兜底：用 UI 脚本的假 push 通道回放录好的 ChatEvent 序列，断言 `reduce.ts` 的 `view()` 快照逐字不变，接进 `npm run check`。基线点名比对 T.1 的五处 no-op 改动
