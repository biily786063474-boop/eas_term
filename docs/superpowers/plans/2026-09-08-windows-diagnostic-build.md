...
- [ ] 5. Add diagnostic-only build identity and disable updater/anonymous telemetry for that identity; add private branch Windows workflow that cannot publish. Add CI verification script for startup marker, cancel/confirm, export, structured events. Run typecheck, full tests, build and real UI verification.
...
- [ ] 7. Push diagnostic branch only, use Windows CI (no tag/main push), retrieve artifact exe and SHA256, record test limits and deliver local installer path.
