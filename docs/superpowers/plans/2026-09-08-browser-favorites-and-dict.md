...
## 执行收口（2026-09-09）
以最终横排覆盖为准，Task5/5A旧OGL清单作废。Task0/1/2/3/4/6已实现并有真实应用回归：路由+表单、自定义目录与贴纸、收藏闭环、仅本地授权截图、数据损坏/软链接防护、重启持久 Cookie/收藏。未把第三方网站登录测试冒充真实账号登录；使用本机 HTTP fixture。
Task6A：全局检查了四个最大化宿主、延迟holder、CanvasStage祖先隐藏与几何、GPU trace；保留布局与收回恒定内容布局方案进入本轮。去模糊/常驻图层实验无改善已弃用。验证记录含前后帧间隔，不能表述为所有机器绝对零掉帧。
Task7：本轮交付隔离开发实例验收，不发版、不替换 0.4.87、不触发 Windows 分发；Windows 实机与正式打包仍是将来发版的独立门槛。Motion Sites/短视频助手未知网址仍明确 pending，不猜测。真实公共缩略图未批量抓取；无本地预览显示明确占位，用户可在收藏时选择保存自己的页面预览。

2026-09-09 发版授权后续：0.4.88 已完成 Mac 双架构与 Windows 正式打包回归并上线，覆盖上述“未发版”的历史状态；见 docs/verification/releases/0.4.88.md。
