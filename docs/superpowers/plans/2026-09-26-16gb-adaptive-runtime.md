...
## 续批 1 / 2 / 5（用户“OK做125”，不是上文 Task 编号）
- 1：按需数字进程树统计，包含活跃 CLI/插件后代；有界读取/隐私/限频/真实子进程验收。
- 2：隔离实例冷启、3分钟空闲、3 Frame、真实图片+3D、关闭后3分钟；真实 Claude 首发受登录前置限制，不用事件回放冒充。
- 5：合成失败/取消结束不得通知成功或计成功用量；复用 interrupted/usageKnown，保留已有忙碌状态清理及恢复链路。
- 仍不包含16GB实体适配结论、热点20%收益、Windows现场验收、合并/发版。
