...
export function createFiles(config){
 ...
 return {
  ...
  write(args){
   ...
   if(!args||Object.keys(args).some(k=>!['path','buffer','expectedSha256'].includes(k))||!Buffer.isBuffer(args.buffer)||Buffer.byteLength(args.buffer)>MAX)throw Error('XLSX参数无效或超过8MB')
