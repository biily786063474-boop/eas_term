...
const tools=[
 {name:'word_read',description:'读取DOCX正文顶层段落和SHA256，不包含表格/页眉/文本框；不执行宏或外部链接。',inputSchema:{type:'object',properties:{path:pathSchema},required:['path'],additionalProperties:false},annotations:{readOnlyHint:true}},
 {name:'word_create',description:'创建DOCX，支持标题、标题级别1–6、粗体和斜体；覆盖须提供当前SHA256。',inputSchema:{type:'object',properties:{path:pathSchema,title:{type:'string'},paragraphs:{type:'array',maxItems:1000,items:{type:'object',properties:{text:{type:'string'},heading:{type:'integer',minimum:1,maximum:6},bold:{type:'boolean'},italic:{type:'boolean'}},required:['text'],additionalProperties:false}},expectedSha256:{type:'string'}},required:['path','paragraphs'],additionalProperties:false},annotations:{readOnlyHint:false,destructiveHint:true}},
...
async function call(name,args){
 ...
 if(name==='word_create')return files.write({path:args.path,buffer:await createDocument({paragraphs:args.paragraphs,...(args.title===undefined?{}:{title:args.title})}),...(args.expectedSha256===undefined?{}:{expectedSha256:args.expectedSha256})})
