...
export function createFiles(config){
 ...
 function read(rel){const file=target(rel),stat=fs.statSync(file);if(!stat.isFile()||stat.size>MAX)throw Error('仅支持不超过1MB的普通文件');const fd=fs.openSync(file,fs.constants.O_RDONLY|fs.constants.O_NOFOLLOW);try{const opened=fs.fstatSync(fd);if(opened.dev!==stat.dev||opened.ino!==stat.ino)throw Error('文件已变化');const b=fs.readFileSync(fd);if(b.length>MAX)throw Error('文件过大');return b}finally{fs.closeSync(fd)}}
 return {
  list(args={}){
  ...
  read(args){if(!args||Object.keys(args).some(k=>k!=='path'))throw Error('参数无效');const b=read(args.path);return {text:new TextDecoder('utf-8',{fatal:true}).decode(b),sha256:hash(b)}},
  write(args){
   if(config.root.access!=='read-write')throw Error('目录只读')
   if(!args||Object.keys(args).some(k=>!['path','text','expectedSha256'].includes(k))||typeof args.text!=='string'||Buffer.byteLength(args.text)>MAX)throw Error('文本参数无效或超过1MB')
   ...
   try{
    fs.writeFileSync(tmp,args.text,{flag:'wx',mode:0o600});target(args.path,true)
    ...
    return {path:args.path,bytes:Buffer.byteLength(args.text),sha256:hash(Buffer.from(args.text))}
