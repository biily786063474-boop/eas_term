...
**不确定变量名叫什么，就先不带参数调一次**：`secret_check({})`

它会列出柜里存了什么 —— 组名、**备注**、变量名（永远不含值）。
备注是用户写给你看的「什么场景用哪条」，按它挑，别猜变量名：
用户存的可能叫 `MY_ALIYUN_AK`，你猜 `ALIYUN_ACCESS_KEY_ID` 就会得到「柜里没有」，
于是去弹窗要一个他早就存好的密钥 —— 那是这个功能最让人恼火的失败方式。

列表为空时**先看 `locked`**：锁着和真没有长得一模一样。
secret_check 会在需要时直接弹出六位码建柜/解锁窗口，解锁后继续原请求。取消后停止索要，不要让用户自己寻找入口，也不要把锁定当作「柜里没有」。

直接创建的 agent 只持有会话通行证，不注入业务密钥；使用 eas-secret run。hasCredential 为 false 表示节点缺少通行证，需要重开节点。未授权组用 request_secret 当场批准；已有变量只授权，无需重新填写。
