...
export function resolveCandidate(cwd,id,decision){if(!['confirm','ignore'].includes(decision))throw Error('候选操作无效');return change(cwd,d=>{const c=d.items.find(x=>x.id===id);if(!c)throw Error('候选不存在');if(c.review!=='pending')return {changed:false};let receipt;if(decision==='confirm')receipt=record(cwd,{taskKey:'capture:'+c.id,title:c.title,summary:c.summary,...(c.originalQuestion?{originalQuestion:c.originalQuestion}:{}),date:c.date,status:'pending',author:'自动捕获 · 用户收录',evidence:[]});c.review=decision==='confirm'?'confirmed':'ignored';return {changed:true,receipt}})}
