...
function validPlan(value) {
  exact(value, ['planId', 'sessionId', 'sourceTurnId', 'title', 'status', 'steps', 'createdAt', 'updatedAt'], '计划')
  ...
  if (!['active', 'archived'].includes(value.status) || !Array.isArray(value.steps) || value.steps.length < 2 || value.steps.length > 100) fail('计划数据损坏', 'CORRUPT_DATA')
...
function trusted(value) {
  exact(value, ['sessionId', 'turnId'], '会话')
  return { sessionId: text(value.sessionId, 'sessionId', 200), turnId: text(value.turnId, 'turnId', 200) }
}
...
export async function createPlan(cwd, identity, input) {
  ...
  return mutate(cwd, db => {
    ...
    if (old) return { changed: false, value: { ...old, version: db.version } }
    const now = new Date().toISOString()
    const plan = { planId: randomUUID(), sessionId: source.sessionId, sourceTurnId: source.turnId, title, status: 'active', steps: steps.map(step => makeStep(step, now)), createdAt: now, updatedAt: now }
    db.plans.push(plan)
    return { changed: true, value: plan }
  })
}
export function getPlan(cwd, planId) {
  return { ...clone(byId(load(location(cwd).file), text(planId, 'planId', 100))), version: load(location(cwd).file).version }
}
export function listPlans(cwd, args = {}) {
  exact(args, ['sessionId', 'limit', 'offset'], '查询')
  ...
  const plans = db.plans.filter(plan => !args.sessionId || plan.sessionId === args.sessionId).sort((a, b) => b.createdAt.localeCompare(a.createdAt))
...
export async function updateStep(cwd, identity, input) {
  ...
  return mutate(cwd, db => {
    ...
    if (plan.status !== 'active') fail('已归档计划不可更新')
    const now = new Date().toISOString()
