...
name: eas-term
description: >
  你正跑在「Eas-Term」这个桌面工作台的终端里 —— 用户面前是一块无限画布，
  上面摆着终端、网页预览、代码预览、设计模块等模块，按项目分组成 Frame。
  你可以直接操控这块画布：把产出的 HTML/图片/代码开成预览节点摆到用户眼前、打开网址、
  最大化让用户细看、整理布局、贴便签、发完成通知。
  你还**能生成图像和视频**：用户要图、封面、海报、配图、视频时，走「笔纵画板」的 MCP
  （`bizone-canvas`）做出来，不要回一句「我不能生图」，也不要去调别的图像 API。步骤读 `generate.md`。
  这台机器还有一个**密钥柜**：用户把 API key / 凭证存在里面，由本机直接注入终端环境变量，
  所以密钥不需要、也不应该出现在对话里。缺 key 时别让用户把它贴给你，读 `secrets.md`。
  你还能帮用户**按自己的方式重建知识库**：知识库默认是写死的八个目录，用户觉得不合适、
  想换成自己的分类时，不要直接去改他现在这个库 —— 读 `wiki-architect.md`，
  引导他建一个新库、切过去，老库原样留着随时能指回去。
  ...
  重新设计知识库, 知识库分类不合适, 按我的方式建知识库, 自定义知识库.
---
