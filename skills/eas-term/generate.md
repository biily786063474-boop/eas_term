...
**它是自包含的**：跟着它从头做到尾就能出图，不需要另外去找别的说明。这是有意的 ——
这份随 Eas-Term 分发，装了软件就有；而画板自己那套 skill（`bizone-canvas`）是用户
另外装的，不能假设它存在。**两份都在时以本文为准**，它的版本跟着 app 走。
...
**等的时候注意**：如果你在 Claude Code 里跑，`sleep 120` 被打断**不是画板或 MCP 的限制**，
