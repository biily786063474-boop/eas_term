...
function fixture(configured=false) {
 let cursor=0; const slots=[],effects=[],calls=[],remembers=[];let resumed=0,finish
 ...
 const api={status:async()=>st,setup:async(c,remember)=>{calls.push('setup');remembers.push(remember);return{ok:true,status:{...st,configured:true,locked:false}}},unlock:(c,remember)=>{calls.push('unlock');remembers.push(remember);return new Promise(r=>finish=r)}}
 ...
 return {render,input,submit,calls,remembers,all,remember(){all(render()).find(n=>n.type==='input'&&n.props.type==='checkbox').props.onChange({target:{checked:true}})},complete(){finish({ok:true,status:{...st,locked:false}})},get resumed(){return resumed},unmount(){effects.forEach(f=>f?.())}}
}
...
test('用户明确勾选后才将信任此设备传给密钥柜',async()=>{
 const f=fixture(true);f.remember();f.input('123456');f.submit()({key:'Enter'});assert.deepEqual(f.remembers,[true]);f.complete();await new Promise(setImmediate)
})
