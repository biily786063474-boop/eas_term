...
export function PluginConfigurationControls({plugin}:{plugin:PluginInfo}):JSX.Element{
 ...
 return <div className="pm-auth" data-plugin-config={plugin.id}>
  ...
  {open&&<form autoComplete="off" onSubmit={e=>{e.preventDefault();void run('save')}}>
   ...
   <div className="pm-auth-actions"><button type="submit" disabled={busy||!Object.keys(draft).length}>保存配置</button><button type="button" disabled={busy} onClick={()=>void run('status')}>刷新配置状态</button><button type="button" disabled={busy||!!Object.keys(draft).length} onClick={()=>void run('test')}>测试连接</button><button type="button" disabled={busy} onClick={()=>void run('clear')}>断开并清除配置</button></div>
