// 画布模式**左侧**资源抽屉：项目列表 + 文件树。整体可收起、分区可折叠。
// （2026-08-13 左右对调过：这个抽屉从右挪到了左，知识库抽屉从左挪到了右。
//  实际定位见 canvas.css 的 `.canvas-drawer { left: 8px }` 与 `.wiki-drawer { right: 8px }`。）
// 拖项目 → 落画布生成 Frame（画布上已有则聚焦）；双击项目 → 新开终端。
