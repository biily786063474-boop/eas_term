...
function RegionWireframe({block,x,y,w,h,mobile}:{block:string;x:number;y:number;w:number;h:number;mobile:boolean}):JSX.Element{
 ...
 const bar=(yy:number,width=inner)=> <path key={yy} d={`M${left} ${yy}h${width}`}/>
 ...
 switch(block){
  ...
  case '金刚区': marks=<>{[0,1,2,3].map(i=><g key={i}>{tile(left+i*inner/4+5,top,10,10)}{bar(top+17,Math.min(16,inner/4-8))}</g>)}</>;break
  ...
  case '页脚': marks=<>{bar(y+h-10,inner*.25)}{bar(y+h-10,inner*.18)}</>;break
