    ...
    <div className="ac-history-body">
      ...
      <section className="ac-history-detail" aria-label="对话预览">
        {selected ? <>
          ...
          <div className="ac-history-transcript">{reading ? <p className="ac-history-empty">正在读取正文…</p> : <MessageList historyPreview view={{ model: null, quotas: [], turns, pending: null, notices: [], usage: null, busy: false }} onApprovalDecide={() => undefined} leafId={leafId} />}</div>
          <footer><small>早期记录可能经过截断，预览不代表模型仍保有上下文。</small><div><button type="button" disabled={working} onClick={() => void pin()}>{selected.pinned ? '取消置顶' : '置顶'}</button><button type="button" disabled={working || reading || !canResume || !selected.resumeId || selected.leafId === currentKey} title={!canResume ? '请先保存并新建一个空对话，再选择历史' : !selected.resumeId ? '缺少原会话标识，仅可查看记录' : undefined} onClick={() => { setWorking(true); void onResume(selected).catch(() => setError('恢复失败，原记录未删除')).finally(() => setWorking(false)) }}>继续此对话</button></div>{!canResume && <small>当前模块有对话，请先新建后再恢复。</small>}</footer>
