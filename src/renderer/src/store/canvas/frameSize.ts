 ...
 *    标题 ~18 + **一行**三颗按钮（~86 高）+ 「先开个终端」~22 + 上下 padding 32
 *  再加上 Frame 头部（HEAD=34）。
 *
 *  宽度按「**三颗挤在一行的最窄可读宽度**」算：2026-09-03 用户要求
 *  「无论什么情况下，他们三个按钮都应该在一行」，所以按钮改成 `flex: 1 1 0`
 *  一起缩、名字过长省略。三颗各 ~76 + 两道 8px 间距 + 左右 padding 32 ≈ 300。
 *
 *  ⚠️ **改 `FrameStart` 的排版时回来核一次这两个数。** 它们之间没有代码约束，
 *  只有这条注释 —— 对不上的表现就是用户看到过的那个溢出。 */
export const EMPTY_FRAME_MIN = { w: 300, h: 268 }
