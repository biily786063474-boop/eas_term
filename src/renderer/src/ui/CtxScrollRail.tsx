 ...
 *
 * ── 不只给双击菜单用 ──────────────────────────────────────────────────────
 * 任何「自己滚、且圆角明显」的容器都能挂它（设置面板那两个名单就在用）。
 * 定位走宿主的 `getBoundingClientRect()` ＋ `position: fixed`，
 * 所以**宿主本身是不是 fixed 无所谓** —— 它只要是个能量出屏幕坐标的盒子。
 *
 * ⚠️ 但**渲染位置要挑**：它虽然是 fixed，只要祖先有 `overflow: hidden`
 * 就一样会被裁掉（设置面板那层灯箱就有）。挂在不被裁的那一层上，
 * 宿主由 ref 指过去 —— 跟 DOM 位置无关。
 *
