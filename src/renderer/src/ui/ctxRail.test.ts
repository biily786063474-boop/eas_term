...
test('起点在顶边距右端 1/3 宽处，**终点在右边到底**', () => {
  const d = railPath({ w: 300, h: 600, r: 20 })
  assert.match(d, /^M 200 0 /, '起点不对（应当是 w*2/3）')
  // 到底 = 右下角圆弧的起点（h - r）。**不能停在半空** ——
  // 滑块贴到一个悬空的末端时，人读到的是「下面还有，只是走不动了」。
  assert.match(d, / V 580$/, '终点不对（应当是 h - r，也就是右边到底）')
})
