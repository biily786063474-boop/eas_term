...
export function railPath({ w, h, r }: RailGeom): string {
  ...
  // 终点：**右边到底** —— 停在右下角圆弧的起点（再往下就拐弯了）。
  //
  // 第一版按用户最初说的「到 2/3 高处止」做，他看过实机后改了主意：
  // 「不然容易误会没滚到头」。他是对的 —— 滑块贴到轨道末端时，
  // 如果那个末端悬在半空，人读到的是「下面还有，只是走不动了」。
  // **轨道的终点就是「到头了」这个信息本身**，必须落在结构上就是尽头的位置。
  const endY = Math.max(h - rr, rr)
