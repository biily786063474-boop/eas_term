...
test('握手中调整方向会撤销原消息，握手完成后只发送新方向', async () => {
  const h = harness()
  h.live.deliver('旧任务')
  assert.equal(h.live.interrupt(), true)
  assert.equal(h.events.filter(e => e.k === 'turn.done').length, 1)
  h.live.deliver('新方向')
  h.f.reply('initialize', {})
  await tick()
  h.f.reply('session/new', NEW_OK())
...
test('首轮参数设置中调整方向必须等配置完成，旧任务不发送', async () => {
  const h = harness({ model: 'p/m2' })
  h.live.deliver('旧任务')
  h.f.reply('initialize', {})
  await tick()
  h.f.reply('session/new', NEW_OK())
  await tick()
  assert.ok(h.f.last('session/set_config_option'))
  assert.equal(h.live.interrupt(), true)
  h.live.deliver('新方向')
  await tick()
  assert.equal(h.f.last('session/prompt'), undefined)
  h.f.reply('session/set_config_option', {})
  await tick()
  const prompts = h.f.sent.filter(m => m.method === 'session/prompt')
  assert.equal(prompts.length, 1)
  assert.deepEqual((prompts[0].params as { prompt: unknown }).prompt, [{ type: 'text', text: '新方向' }])
