...
import {installSessionStartup,startManagedSession,cancelSessionStart} from '../../runtime/sessionStartup.ts'
...
test('real OMP admission cancels before spawn, then admits a fresh message and holds budget until close', {
  ...
  const manager=createRuntimeManager({now:Date.now});installSessionStartup(manager)
  let sequence=0,spawns=0
  ...
  const model = http.createServer((req, res) => {
    req.resume(); req.on('end', () => {
      ...
      chunk('', null)
      // All responses are local fixtures; no account credentials or remote model.
  ...
  const live = createAcpLive({
    ...
    async openAsync(cwd,signal) {
      const id='real-acp:'+ ++sequence
      const cancel=()=>cancelSessionStart(id,1)
      signal.addEventListener('abort',cancel,{once:true})
      try{return await startManagedSession<ReturnType<typeof this.open>>({id,windowId:1,name:'fixture',projectId:'fixture',cost:{cpu:7,memoryBytes:512*1024**2},start:async admitted=>{
        if(admitted.aborted||signal.aborted)throw Error('cancelled')
        const value=this.open(cwd)
        if(!value.ok)throw Error(value.message)
        return {value,completed:value.proc.completed!}
    ...
    open() {
      ...
      return { ok: true, proc: { completed:new Promise<void>(r=>p.once('close',()=>r())), write: s => { p.stdin.write(s) }, onLine: cb => { lines.on('line', cb) },
  ...
  try {
    live.deliver('Never dispatch');await new Promise(r=>setImmediate(r))
    assert.equal(spawns,0);assert.equal(live.interrupt(),true)
    await new Promise(r=>setImmediate(r));assert.equal(spawns,0);assert.equal(requests,0)
    manager.update({at:Date.now(),cpu:1,memoryUsedBytes:1,totalMemoryBytes:8*1024**3,critical:false})
    live.deliver('New explicit message');await until(()=>requests===1)
    assert.equal(spawns,1);assert.equal(manager.snapshot().running,0)
    assert.equal(manager.snapshot().reserved.memoryBytes,512*1024**2)
    await until(()=>live.phase()==='ready')
    assert.ok(JSON.stringify(events).includes('REDIRECT_SURVIVED'))
    live.close();await until(()=>child?.exitCode!==null||child?.signalCode!==null)
    await until(()=>manager.snapshot().reserved.memoryBytes===0)
    assert.equal(killed,1);assert.equal(requests,1)
  } finally {
    live.close()
