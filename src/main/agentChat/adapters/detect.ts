...
/** POSIX 用 `which`；Windows Codex/Claude 用与实际启动相同的包元数据解析器，
 *  避免 `where` 只找到了不可执行的 npm .cmd 却判成可用。
...
export function detectByWhich(bin: string): () => Promise<boolean> {
  if (process.platform === 'win32' && ['claude', 'codex'].includes(bin)) return async () => {
    try { cliInvocation(bin, bin, []); return true } catch { return false }
  }
  return () =>
    new Promise((resolve) => {
