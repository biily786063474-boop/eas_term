...
export const BUILTIN_ROLES: AgentRole[] = [
  {
    ...
    contract: [
      ...
      '每完成一个可交付的小步，用 board_note 往你这条分支的台账记一条（决定 / 未完成 / 给合并官 / 怎么验）；动手前读一遍系统提示里指的章程。'
    ].join('\n')
  },
  {
    id: 'scout',
  ...
  },
  {
    ...
    contract: [
      ...
      '流程固定：1 用 board_read 看协同板，**读目标分支台账里「给合并官」的条目**；2 用 merge_preflight 看主干名（defaultBranch）、改动文件、冲突文件、别的分支是否在改同一文件、回归命令（testCmd）；',
      ...
      '合并成功后用 board_note（带 branch=<那条分支>）写一条「已合并进 <主干>（<merge 提交 sha>）」。'
    ].join('\n')
  },
  {
    id: 'prototyper',
