...
test('official npm Windows shim resolves before a PTY lease is requested', t => {
  ...
  fs.writeFileSync(path.join(pkg, 'package.json'), JSON.stringify({ name: '@anthropic-ai/claude-code', version: '0.1.0', bin: { claude: 'cli.js' } }))
  fs.writeFileSync(path.join(pkg, 'cli.js'), 'console.log("fixture")')
  const shim = path.join(root, 'claude.cmd'); fs.writeFileSync(shim, 'DO NOT EXECUTE SHIM TEXT')
  assert.equal(resolveCapabilityCli('claude', { PATH: 'C:\\npm', PATHEXT: '.CMD' }, 'win32', () => shim), shim)
})
