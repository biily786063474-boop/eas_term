...
test('production app shutdown and window cleanup route both soft/hard passes through owned control',()=>{
 ...
 const api=runInNewContext(code+'\n({killAllAgentChatSessions,killAgentChatSessionsForWebContents})',{timelineRuntime,sessions,stopAgentProcess,revokeCapabilitySession(){},transcripts:{drop(){}},setTimeout(fn){timers.push(fn);return {unref(){}}}})
 ...
 assert.deepEqual(messages.map(m=>m.signal),['SIGTERM','SIGKILL','SIGTERM','SIGKILL']);assert.equal(sessions.size,0)
})
