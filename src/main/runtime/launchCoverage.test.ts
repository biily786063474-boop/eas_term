...
const MANIFEST: Record<string, Entry> = {
  ...
  'src/main/mcpClient.ts': { status: 'managed', counts: { spawn: 1 }, note: '两条路共用这一处 spawn，都已准入：插件宿主 pluginHost.acquire 经应用级 startManagedSession；builtin-bizone 连接器 bizoneConnector.clientFor 经 deps.admit（bizoneHosted 注入应用级 startManagedSession，**interactive:true** —— 同插件 server/终端，用户触发的一次性共享启动立即起、不排队，成本照登记）。预算都随 McpClient.exited 释放。笔纵客户端进程在运行中心只见排队任务、不作为服务投影（builtin 宿主不在 observedPluginServices 里）' },
