...
test('runtime stop routes owned ASR to the shared registry and retains confirmation/ownership checks',async()=>{
 ...
 const start=source.indexOf(" ipcMain.handle('runtime:stopPlugin'")
 ...
 const deps={ipcMain:{handle:(_n:string,fn:any)=>handler=fn},BrowserWindow:{fromWebContents:()=>win},sharedServices:registry,ownedSessions:{stop:()=>{throw Error('wrong owned route')}},stopObservedPlugin:()=>{throw Error('wrong plugin route')},stopGate:(_id:string,fn:()=>unknown)=>fn(),dialog:{showMessageBox:async(_win:unknown,opts:{detail:string})=>{detail=opts.detail;confirmations++;return {response:1}}},runtimeProjectLabels:()=>[],projectsSource:()=>[]}
