...
export function createRuntimeController(deps:{mode?:ResourceMode;now:()=>number;read:()=>Promise<Reading>;setTimer:(fn:()=>void,ms:number)=>unknown;clearTimer:(handle:unknown)=>void}){
 ...
 return {readForControl():RuntimeMonitorSnapshot{
   ...
   return {...(latest??{sampledAt:0,logicalCpus:0,totalMemoryBytes:0,memoryMethod:'unavailable',enforcement:'plugin-tools' as const}),
    metricsAvailable:fresh,cpuPercent:fresh?latest!.cpuPercent:null,memoryUsedBytes:fresh?latest!.memoryUsedBytes:null,mode:policy.mode,threshold:policy.threshold}
