...
export function createHttpsSender(trust:Pick<ConnectionOptions,'ca'>={}) {
 return async(plan:ConnectionPlan,init:RequestInit):Promise<Response>=>new Promise((resolve,reject)=>{
  const agent=plan.proxy?new HttpsProxyAgent(plan.proxy,{keepAlive:false,...trust}):new https.Agent({keepAlive:false,...trust})
  const headers=Object.fromEntries(new Headers(init.headers).entries())
  const req=https.request({hostname:plan.address,port:plan.url.port||443,servername:plan.servername,path:plan.url.pathname+plan.url.search,method:init.method||'GET',headers,agent,...trust,signal:init.signal||undefined},res=>{
   ...
   limit.on('close',()=>{res.destroy();agent.destroy()})
   const status=res.statusCode||502
   if([204,205,304].includes(status)){res.resume();res.once('end',()=>agent.destroy());resolve(new Response(null,{status,headers:responseHeaders}));return}
   res.pipe(limit)
