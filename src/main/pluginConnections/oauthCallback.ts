...
interface AuthorizationCode {code:string;issuer:string;resource:string;redirectUri:string;verifier:string}
...
export async function startOAuthCallback(options:Options) {
 ...
 const server=http.createServer({maxHeaderSize:8192},(req,res)=>{
  ...
  resolve({code:code!,issuer,resource,redirectUri,verifier})
 })
 ...
 return {state,verifier,challenge,redirectUri,result,cancel}
}
