...
const api = {
  ...
  plugins: {
    configuration: (action: 'status'|'save'|'directory'|'test'|'clear', id: string, values?: Record<string,string|null>|string): Promise<{ok:true;configured:string[];tools?:number}|{ok:false;error:string}> => ipcRenderer.invoke('plugins:configuration', {action,id,values}),
