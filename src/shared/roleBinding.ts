...
/** 给 Codex 显式装配本次基础能力与所选业务插件的 MCP 配置。
 * 不依赖全局注册是否存在；只设置连接字段，不覆盖 enabled/disabled_tools。
 * envVars 仅转发父进程变量名，凭证值不能进入命令行。 */
export function codexAddServerArgs(server: {
  ...
  // Compatibility fallback: preserve Codex's existing native remote registration.
  // A partial -c stdio override would destroy its URL/auth configuration.
  if (server.nativeRemote) return []
  const esc = (s: string): string => s.replace(/\\/g, '\\\\').replace(/"/g, '\\"')
  ...
  if (server.envVars?.length) out.push(`${k}.env_vars=[${server.envVars.map(n => JSON.stringify(n)).join(',')}]`)
  return out
}
