...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      '（分支名 → 文本；板上有的和磁盘上留着的（会话已停的分支也在，合并官要的正是它们），头部由 app 维护，看「## 记录」段有没有内容）' +
  ...
  },
  {
    name: 'dict_add',
    description:
      '往用户的辞典里添加词条。**格式必须和内置的 381 条完全一致**，缺字段会被拒收——' +
      '半截词条（只有英文名、没解释没图）混进去既帮不上忙又稀释整本辞典的可信度。' +
      '每条必填:zh 中文名 + en 英文名 + category(interaction/motion/visual) + ' +
      'logic(80-140 字讲清是什么、怎么实现) + svg(240x120 极简示意图)。' +
      '强烈建议同时给:cat1/cat2(二级分类，必须是真实存在的一对，写错当场拒) + ' +
      'prompt(点击后挂进对话的完整提示词，没有它这条词点下去只能插解释)。' +
      '**先读 eas-term skill 的 dict.md**——那里有分类表、SVG 的四条硬约束(比如 <style> 会被整个删掉)、' +
      '提示词的六段式规格。别凭印象写。' +
      '拿不准的词直接跳过，宁缺毋滥。返回的 rejected 里会说明哪些被拒、为什么——' +
      'ok:true 不代表全进去了，逐条看。',
    inputSchema: {
      type: 'object',
      properties: {
