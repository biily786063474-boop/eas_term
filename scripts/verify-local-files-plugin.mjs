...
try{
 ...
 check(await main.eval("(()=>{const b=document.querySelector('[data-plugin-config] button');return !!b.closest('.pm-cact')&&b.textContent.includes('授权目录')&&b.getAttribute('title').includes('文件夹')})()"),'授权入口位于右侧独立操作区，名称与用途明确')
 ...
 await main.eval("document.activeElement.blur()")
 await main.send('Input.dispatchMouseEvent',{type:'mouseMoved',x:10,y:10})
 check(await main.eval("getComputedStyle(document.querySelector('[data-plugin-config] .pm-settings-trigger')).opacity==='0'"),'pointer leaves card: settings trigger is hidden')
 const idleShot=await main.send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(path.join(output,'market-idle.png'),Buffer.from(idleShot.data,'base64'))
 const idleBodyWidth=await main.eval("document.querySelector('[data-plugin-config]').closest('.pm-card').querySelector('.pm-cb').getBoundingClientRect().width")
 const railBefore=await main.eval("(()=>{const r=document.querySelector('[data-plugin-config]').closest('.pm-card').getBoundingClientRect();return {width:r.width,height:r.height}})()")
 const hoverTarget=await main.eval("(()=>{const r=document.querySelector('[data-plugin-config] button').getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2}})()")
 await main.send('Input.dispatchMouseEvent',{type:'mouseMoved',...hoverTarget})
 await until(()=>main.eval("getComputedStyle(document.querySelector('[data-plugin-config] .pm-settings-trigger')).opacity==='1'"))
 await until(()=>main.eval("getComputedStyle(document.querySelector('[data-plugin-config] .pm-setting-tip')).visibility==='visible'"))
 check(await main.eval("getComputedStyle(document.querySelector('[data-plugin-config] .pm-setting-tip')).visibility==='visible'"),'pointer hover reveals purpose tooltip')
 const railAfter=await main.eval("(()=>{const r=document.querySelector('[data-plugin-config]').closest('.pm-card').getBoundingClientRect();return {width:r.width,height:r.height}})()")
 check(JSON.stringify(railBefore)===JSON.stringify(railAfter),'hover does not change card dimensions')
 check(await main.eval("(()=>{const c=document.querySelector('[data-plugin-config]').closest('.pm-card'),a=c.querySelector('.pm-setting-entry');return Math.abs(a.getBoundingClientRect().height-c.getBoundingClientRect().height)<=2&&getComputedStyle(a).position==='absolute'})()"),'hover action panel fills card height without reserving layout space')
 check(await main.eval("document.querySelector('[data-plugin-config]').closest('.pm-card').querySelector('.pm-cb').getBoundingClientRect().width")<idleBodyWidth,'default content uses space released by hidden panel')
