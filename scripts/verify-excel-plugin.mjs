...
import {readWorkbook} from '../plugins-store/excel/lib/workbook.mjs'
...
app.whenReady().then(()=>session.defaultSession.webRequest.onBeforeRequest({urls:['https://eas.biily.top/plugins/excel/*']},(details,callback)=>callback({redirectURL:${JSON.stringify(serverUrl)}+new URL(details.url).pathname})));
...
try{
 ...
 await until(()=>main.eval("[...document.querySelectorAll('.pm-card')].some(e=>e.textContent.includes('Excel 表格'))"))
 await main.eval("[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('Excel 表格')).querySelector('button').click()")
 await until(()=>main.eval("[...document.querySelectorAll('button')].some(e=>e.textContent.includes('确认安装'))"))
 check(!fs.existsSync(path.join(home,'.eas/plugins/excel/plugin.json')),'Excel 表格包安装前需真实市场确认')
 ...
 await main.eval("document.querySelector('[data-plugin-config=\"eas:excel\"] button').click()")
 ...
 for(const action of ['save','directory']){
  ...
  await main.eval('window.__pendingConfig=window.api.plugins.configuration('+JSON.stringify(action)+',"eas:excel",'+(action==='save'?'{}':'"root"')+');void 0')
 ...
 await main.eval('window.__pendingConfig=window.api.plugins.configuration("save","eas:excel",{});void 0')
 ...
 const cleared=await main.eval('window.api.plugins.configuration("status","eas:excel")')
 check(cleared.ok&&cleared.configured.length===0&&fs.readdirSync(allowed).length===3,'清除真实配置但保留三个业务文件')
 let revoked=false;try{revoked=!!(await clients[0].request('tools/call',{name:'excel_create',arguments:{path:'after-clear.xlsx',sheets:[{name:'x',rows:[['forbidden']]}]}})).isError}catch{revoked=true}
 check(revoked&&!fs.existsSync(path.join(allowed,'after-clear.xlsx')),'清除使旧shim连接不能继续写入')
 await main.eval("[...document.querySelectorAll('[data-plugin-config] button')].find(e=>e.textContent.includes('选择目录')).click()")
 await until(()=>main.eval("document.querySelector('[data-plugin-config]').innerText.includes('已保存')"))
 await clients[0].initialize('0.4.102')
 const restored=await clients[0].request('tools/call',{name:'excel_read',arguments:{path:'claude.xlsx'}})
 check(!restored.isError,'重新授权后可以重新建立连接读取原业务文件')
 await main.eval('window.api.secrets.lock()');await wait(500)
 let blocked=false;try{const r=await clients[0].request('tools/call',{name:'excel_create',arguments:{path:'after-lock.xlsx',sheets:[{name:'x',rows:[['must-not-write']]}]}});blocked=!!r.isError}catch{blocked=true}
 check(blocked&&!fs.existsSync(path.join(allowed,'after-lock.xlsx')),'密钥柜锁定关闭实际配置插件，后续调用不能写入')
