...
VER=$(defaults read "$NEW/Contents/Info.plist" CFBundleShortVersionString 2>/dev/null)
[ -n "$VER" ] || { bad "新包读不到版本号"; exit 1; }

# 还开着就别装 —— 这是这个脚本存在的全部理由
if ps -axo command | grep -q "[E]as-Term.app/Contents/MacOS/Eas-Term"; then
...
OLD=unknown
[ -d "$APP" ] && OLD=$(defaults read "$APP/Contents/Info.plist" CFBundleShortVersionString 2>/dev/null || echo unknown)
if [ -d "$APP" ]; then
  BAK="$APP.old-$OLD"
  [ -e "$BAK" ] && rm -rf "$BAK"
...
GOT=$(defaults read "$APP/Contents/Info.plist" CFBundleShortVersionString 2>/dev/null)
if [ "$GOT" != "$VER" ]; then
