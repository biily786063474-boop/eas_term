...
try {
  ...
  for(const name of ['首页','控制台 / 后台']){
   ...
   check(await evaluate("!!document.querySelector('.bp-diagram [role=button]')&&document.querySelector('.bp-location').textContent.includes('最上方')"),name+'SVG位置说明可见')
   ...
   check(await evaluate("new Set([...document.querySelectorAll('.bp-diagram .bp-region')].map(e=>getComputedStyle(e).getPropertyValue('--bp-color'))).size>1"),name+'SVG区块使用不同配色')
  ...
  await until(()=>evaluate("!!document.querySelector('.bp-diagram')"));await shot('blueprint-light')
