...
try{
 ...
 let workbook=Buffer.from((await runEngine(root,{operation:'create',sheets:[{name:'Sheet1',rows:[['Region','Amount',null,{formula:'SUM(B2:B3)'}],['East',10],['West',20]]}]})).workbook,'base64')
 const initial=await runEngine(root,{operation:'read',workbook:workbook.toString('base64')});assert.equal(initial.sheets[0].rows[1][1],10);assert.equal(initial.calculated,false)
 ...
 const result={passed:true,checks:8,actualBinary:true,parentAdapter:true,platform:process.platform,arch:process.arch,pluginIntegrated:false,excelVisualVerified:false}
