// Preparation: npm ci --prefix scripts/powerpoint-connector --ignore-scripts
// Runtime package is fully offline; never run npm/npx on user installation.
import {buildSync} from 'esbuild'
import fs from 'node:fs'
import path from 'node:path'
const source='scripts/powerpoint-connector',target='plugins-store/powerpoint'
const result=buildSync({metafile:true,entryPoints:[source+'/presentation.mjs'],bundle:true,platform:'node',format:'cjs',target:'node22',outfile:target+'/lib/presentation.cjs',legalComments:'external'})
// Upstream source comments contain blank lines with trailing spaces.
const bundle=target+'/lib/presentation.cjs'
...
const embedded=target+'/lib/presentation.cjs.LEGAL.txt'
...
console.log('Bundled PowerPoint connector and '+notices.length+' dependency notices')
