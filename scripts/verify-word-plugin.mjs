...
import {readDocument} from '../plugins-store/word/lib/document.mjs'
...
const root=process.cwd(),output=path.join(root,'docs/verification/plugin-marketplace/word');fs.mkdirSync(output,{recursive:true})
const fixture=fs.mkdtempSync(path.join(os.tmpdir(),'eas-word-vertical-')),profile=path.join(fixture,'profile'),home=path.join(fixture,'home'),allowed=path.join(fixture,'allowed')
for(const dir of [profile,home,allowed])fs.mkdirSync(dir)
fs.writeFileSync(path.join(profile,'projects.json'),JSON.stringify([{id:'picker-fixture',name:'Word 文档验收',path:fixture}]))
...
const packed=packPlugin('plugins-store/word',{outRoot:path.join(fixture,'packages'),registrySchema:2})
...
app.whenReady().then(()=>session.defaultSession.webRequest.onBeforeRequest({urls:['https://eas.biily.top/plugins/word/*']},(details,callback)=>callback({redirectURL:${JSON.stringify(serverUrl)}+new URL(details.url).pathname})));
...
try{
 ...
 await until(()=>main.eval("[...document.querySelectorAll('.pm-card')].some(e=>e.textContent.includes('Word 文档'))"))
 await main.eval("[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('Word 文档')).querySelector('button').click()")
 await until(()=>main.eval("[...document.querySelectorAll('button')].some(e=>e.textContent.includes('确认安装'))"))
 check(!fs.existsSync(path.join(home,'.eas/plugins/word/plugin.json')),'Word 文档包安装前需真实市场确认')
 await main.eval("[...document.querySelectorAll('button')].find(e=>e.textContent.includes('确认安装')).click()")
 await until(()=>main.eval("!!document.querySelector('[data-plugin-config=\"eas:word\"]')"))
 await main.eval("document.querySelector('[data-plugin-config=\"eas:word\"] button').click()")
 ...
 for(const action of ['save','directory']){
  ...
  await main.eval('window.__pendingConfig=window.api.plugins.configuration('+JSON.stringify(action)+',"eas:word",'+(action==='save'?'{}':'"root"')+');void 0')
 ...
 await main.eval('window.__pendingConfig=window.api.plugins.configuration("save","eas:word",{});void 0')
 ...
 const cleared=await main.eval('window.api.plugins.configuration("status","eas:word")')
 check(cleared.ok&&cleared.configured.length===0&&fs.readdirSync(allowed).length===3,'清除真实配置但保留三个业务文件')
 let revoked=false;try{revoked=!!(await clients[0].request('tools/call',{name:'word_create',arguments:{path:'after-clear.docx',paragraphs:[{text:'forbidden'}]}})).isError}catch{revoked=true}
 check(revoked&&!fs.existsSync(path.join(allowed,'after-clear.docx')),'清除使旧shim连接不能继续写入')
 await main.eval("[...document.querySelectorAll('[data-plugin-config] button')].find(e=>e.textContent.includes('选择目录')).click()")
 await until(()=>main.eval("document.querySelector('[data-plugin-config]').innerText.includes('已保存')"))
 await clients[0].initialize('0.4.102')
 const restored=await clients[0].request('tools/call',{name:'word_read',arguments:{path:'claude.docx'}})
 check(!restored.isError,'重新授权后可以重新建立连接读取原业务文件')
 await main.eval('window.api.secrets.lock()');await wait(500)
 let blocked=false;try{const r=await clients[0].request('tools/call',{name:'word_create',arguments:{path:'after-lock.docx',paragraphs:[{text:'must-not-write'}]}});blocked=!!r.isError}catch{blocked=true}
 check(blocked&&!fs.existsSync(path.join(allowed,'after-lock.docx')),'密钥柜锁定关闭实际配置插件，后续调用不能写入')
 const state=await main.eval('window.api.plugins.configuration("status","eas:word")');check(!state.ok&&!('values' in state),'锁定后配置IPC不返回明文')
 fs.writeFileSync(path.join(output,'result.json'),JSON.stringify({passed:true,checks,scope:'Real market UI install/hash/extraction -> actual app/native-dialog adapter -> real safeStorage -> actual shared host -> three shim subprocesses -> real temp DOCX files. Not Word rendering or actual model CLI or native picker interaction; no production publish.'},null,2))
