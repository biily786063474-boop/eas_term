...
import { verifyMessageQueue } from './verify-message-queue.mjs'
...
main()
  .then(() => {
    if (process.argv.includes('--low-memory') || process.argv.includes('--voice') || process.argv.includes('--spacing') || process.argv.includes('--token-stats')) { log('✓ 所选专项检查通过（未运行通用十一条）'); process.exitCode=0; return }
