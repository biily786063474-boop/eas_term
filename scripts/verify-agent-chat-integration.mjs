...
export async function verifyChatIntegration({ cdp, projectDir, root, waitFor }) {
  ...
  await sendKey()
  check(await cdp.eval(`window.__composerTestSends().length===${sendsBefore} && document.querySelector('[data-composer-input]').value==='' && document.querySelector('.ac-message-queue')?.textContent.includes('完成后立即续发')`),'Codex 生成中快捷键入队，不提前投递')
