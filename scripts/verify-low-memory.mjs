...
const rss=samples.flatMap(s=>typeof s.processTree?.residentBytes==='number'?[s.processTree.residentBytes]:[])
const report={phase,platform:os.platform(),arch:os.arch(),physicalMemoryBytes:os.totalmem(),durationSeconds:seconds,scope:'app main + live descendants; excludes diagnostic helper and reparented daemons; shared RSS may be counted twice; Electron subaggregate is not additive',sampledPeakResidentBytes:rss.length?Math.max(...rss):null,samples}
