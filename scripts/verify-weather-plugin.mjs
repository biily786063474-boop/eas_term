...
const root=process.cwd(),out=path.join(root,'docs/verification/plugin-marketplace/weather')
fs.mkdirSync(out,{recursive:true})
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'eas-weather-')),profile=path.join(tmp,'profile'),home=path.join(tmp,'home')
for(const p of [profile,home])fs.mkdirSync(p)
const name='weather-fixture-'+process.pid,plugin=path.join(tmp,name)
fs.cpSync(path.join(root,'plugins-store/weather'),plugin,{recursive:true})
const manifest=JSON.parse(fs.readFileSync(path.join(plugin,'plugin.json'),'utf8'));manifest.name=name;manifest.displayName='天气隔离验收';fs.writeFileSync(path.join(plugin,'plugin.json'),JSON.stringify(manifest))
fs.writeFileSync(path.join(profile,'projects.json'),JSON.stringify([{id:'picker-fixture',name:'天气验收',path:tmp}]))
...
const server=http.createServer((req,res)=>{
 ...
 calls++;const u=new URL(req.url,'http://fixture');if(u.searchParams.get('key')!=='fixture-weather-key'){res.writeHead(403);res.end();return}res.setHeader('content-type','application/json');res.end(JSON.stringify({status:'1',lives:[{adcode:'110101',city:'fixture',weather:'晴',reporttime:'fixture-current'}],forecasts:[{adcode:'110101',city:'fixture',reporttime:'fixture-forecast',casts:[{date:'2026-09-22',dayweather:'晴'}]}]}))
})
...
try{
 ...
 await until(()=>main.eval("[...document.querySelectorAll('.pm-card')].some(e=>e.textContent.includes('天气隔离验收'))"))
 await main.eval("[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('天气隔离验收')).querySelector('button').click()")
 ...
 check(true,'实际市场下载安装天气包，安装不自动授权')
 ...
 await main.eval('(()=>{const e='+card+'.querySelector("input");Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value").set.call(e,"fixture-weather-key");e.dispatchEvent(new Event("input",{bubbles:true}))})()')
 ...
 const encrypted=fs.readdirSync(path.join(profile,'plugin-credentials'));check(encrypted.length===1&&!fs.readFileSync(path.join(profile,'plugin-credentials',encrypted[0]),'utf8').includes('fixture-weather-key'),'配置真实加密保存')
 check(await main.eval(card+'.querySelector("input").value===""'),'配置保存后不回显Key')
 await click('测试连接');await until(()=>main.eval(card+'.innerText.includes("连接测试通过")'));check(calls===0,'连接测试不消耗天气查询额度')
 ...
 for(const label of ['claude','codex','omp']){
  ...
  await c.initialize('0.4.102');const r=await c.request('tools/call',{name:'weather_current',arguments:{city:'110101'}})
  check(!r.isError&&JSON.parse(r.content[0].text).data[0].reporttime==='fixture-current',label+'真实shim查询实况天气')
 }
 for(const [i,label] of ['claude','codex','omp'].entries()){
  const r=await clients[i].request('tools/call',{name:'weather_forecast',arguments:{city:'110101'}})
  check(!r.isError&&JSON.parse(r.content[0].text).data[0].casts[0].date==='2026-09-22',label+'真实shim查询未来预报')
 }
 check(calls===6,'三个shim分别查询实况和预报')
 const bad=await clients[0].request('tools/call',{name:'weather_current',arguments:{city:'https://127.0.0.1/'}})
 check(bad.isError&&calls===6,'实际宿主调用拒绝内网，不发额外请求')
 await main.eval("[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('天气隔离验收')).scrollIntoView({block:'center'})")
 ...
 let revoked=false;try{revoked=!!(await clients[0].request('tools/call',{name:'weather_current',arguments:{city:'110101'}})).isError}catch{revoked=true}
 check(revoked&&calls===6,'清除配置使旧shim失效，不再携Key查询')
