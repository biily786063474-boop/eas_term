...
try {
 ...
 await main.eval("document.querySelector('.pm-settings-close').click()")
 check(await main.eval("(()=>{const card=[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('待授权审核验收插件'));return !!card&&card.textContent.includes('等待服务商回调审核')&&!card.querySelector('.pm-add')})()"),'v2待接入条目展示原因且无安装按钮')
 await main.eval("[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('兼容性验收插件')).querySelector('.pm-add').click()")
