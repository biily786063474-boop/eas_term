...
import {readPresentation} from '../plugins-store/powerpoint/lib/presentation.mjs'
...
const root=process.cwd(),output=path.join(root,'docs/verification/plugin-marketplace/powerpoint');fs.mkdirSync(output,{recursive:true})
const fixture=fs.mkdtempSync(path.join(os.tmpdir(),'eas-powerpoint-vertical-')),profile=path.join(fixture,'profile'),home=path.join(fixture,'home'),allowed=path.join(fixture,'allowed')
for(const dir of [profile,home,allowed])fs.mkdirSync(dir)
fs.writeFileSync(path.join(profile,'projects.json'),JSON.stringify([{id:'picker-fixture',name:'PowerPoint 演示验收',path:fixture}]))
...
const packed=packPlugin('plugins-store/powerpoint',{outRoot:path.join(fixture,'packages'),registrySchema:2})
...
app.whenReady().then(()=>session.defaultSession.webRequest.onBeforeRequest({urls:['https://eas.biily.top/plugins/powerpoint/*']},(details,callback)=>callback({redirectURL:${JSON.stringify(serverUrl)}+new URL(details.url).pathname})));
...
try{
 ...
 await until(()=>main.eval("[...document.querySelectorAll('.pm-card')].some(e=>e.textContent.includes('PowerPoint 演示'))"))
 await main.eval("[...document.querySelectorAll('.pm-card')].find(e=>e.textContent.includes('PowerPoint 演示')).querySelector('button').click()")
 await until(()=>main.eval("[...document.querySelectorAll('button')].some(e=>e.textContent.includes('确认安装'))"))
 check(!fs.existsSync(path.join(home,'.eas/plugins/powerpoint/plugin.json')),'PowerPoint 演示包安装前需真实市场确认')
 await main.eval("[...document.querySelectorAll('button')].find(e=>e.textContent.includes('确认安装')).click()")
 await until(()=>main.eval("!!document.querySelector('[data-plugin-config=\"eas:powerpoint\"]')"))
 await main.eval("document.querySelector('[data-plugin-config=\"eas:powerpoint\"] button').click()")
 ...
 for(const action of ['save','directory']){
  ...
  await main.eval('window.__pendingConfig=window.api.plugins.configuration('+JSON.stringify(action)+',"eas:powerpoint",'+(action==='save'?'{}':'"root"')+');void 0')
 ...
 for(const label of ['claude','codex','omp']){
  const c=new McpClient({name:'verify-'+label,command:process.execPath,args:[path.join(root,'mcp/eas-plugin-shim.mjs')],cwd:fixture,env:{PATH:process.env.PATH||'',EAS_PLUGIN:'powerpoint',EAS_TERM_PORT:String(endpoint.port),EAS_TERM_TOKEN:endpoint.token}});clients.push(c)
  await c.initialize('0.4.102');check((await c.listTools()).some(t=>t.name==='powerpoint_create'),label+'真实shim经实际应用宿主获得工具')
  const text='actual-'+label,result=await c.request('tools/call',{name:'powerpoint_create',arguments:{path:label+'.pptx',slides:[{title:label,body:text}]}})
  check(!result.isError&&(await readPresentation(fs.readFileSync(path.join(allowed,label+'.pptx')))).slides[0].texts[1]===text,label+'真实工具调用写入授权临时目录')
  const read=await c.request('tools/call',{name:'powerpoint_read',arguments:{path:label+'.pptx'}})
  check(!read.isError&&JSON.parse(read.content[0].text).slides[0].texts[0]===label&&(await readPresentation(fs.readFileSync(path.join(allowed,label+'.pptx')))).slides[0].texts[0]===label,label+'真实宿主往返创建并读取幻灯片文字')
 }
 for(const [i,label] of ['claude','codex','omp'].entries()){
  const read=await clients[i].request('tools/call',{name:'powerpoint_read',arguments:{path:label+'.pptx'}})
  const revision=JSON.parse(read.content[0].text).sha256
  const edit=await clients[i].request('tools/call',{name:'powerpoint_edit',arguments:{path:label+'.pptx',slide:0,textIndex:1,text:'edited-'+label,expectedSha256:revision}})
  check(!edit.isError&&(await readPresentation(fs.readFileSync(path.join(allowed,label+'.pptx')))).slides[0].texts[1]==='edited-'+label,label+'真实宿主按哈希编辑PPTX文字')
 }
 const bad=await clients[0].request('tools/call',{name:'powerpoint_read',arguments:{path:'../profile/secrets.pptx'}});check(bad.isError,'真实宿主调用拒绝越出授权目录')
 const beforeClear=JSON.parse(fs.readFileSync(dialogLog)).length
 fs.writeFileSync(holdDialog,'hold pending save')
 await main.eval('window.__pendingConfig=window.api.plugins.configuration("save","eas:powerpoint",{});void 0')
 ...
 const cleared=await main.eval('window.api.plugins.configuration("status","eas:powerpoint")')
 check(cleared.ok&&cleared.configured.length===0&&fs.readdirSync(allowed).length===3,'清除真实配置但保留三个业务文件')
 let revoked=false;try{revoked=!!(await clients[0].request('tools/call',{name:'powerpoint_create',arguments:{path:'after-clear.pptx',slides:[{title:'forbidden'}]}})).isError}catch{revoked=true}
 check(revoked&&!fs.existsSync(path.join(allowed,'after-clear.pptx')),'清除使旧shim连接不能继续写入')
 await main.eval("[...document.querySelectorAll('[data-plugin-config] button')].find(e=>e.textContent.includes('选择目录')).click()")
 await until(()=>main.eval("document.querySelector('[data-plugin-config]').innerText.includes('已保存')"))
 await clients[0].initialize('0.4.102')
 const restored=await clients[0].request('tools/call',{name:'powerpoint_read',arguments:{path:'claude.pptx'}})
 check(!restored.isError,'重新授权后可以重新建立连接读取原业务文件')
 await main.eval('window.api.secrets.lock()');await wait(500)
 let blocked=false;try{const r=await clients[0].request('tools/call',{name:'powerpoint_create',arguments:{path:'after-lock.pptx',slides:[{title:'must-not-write'}]}});blocked=!!r.isError}catch{blocked=true}
 check(blocked&&!fs.existsSync(path.join(allowed,'after-lock.pptx')),'密钥柜锁定关闭实际配置插件，后续调用不能写入')
 const state=await main.eval('window.api.plugins.configuration("status","eas:powerpoint")');check(!state.ok&&!('values' in state),'锁定后配置IPC不返回明文')
 fs.writeFileSync(path.join(output,'result.json'),JSON.stringify({passed:true,checks,scope:'Real market UI install/hash/extraction -> actual app/native-dialog adapter -> real safeStorage -> actual shared host -> three shim subprocesses -> real temp PPTX files. Not PowerPoint rendering or actual model CLI or native picker interaction; no production publish.'},null,2))
