...
2026-09-18 动态OAuth纵向接通：manifest显式registrationEndpoint与clientId互斥+auth.oauth.dcr要求；pluginAuthorization工厂实际选择动态runtime，登录发现校对全部批准端点，再DCR/PKCE/存储，共享宿主沿用。新增4单测红→绿。6388全量3418/3400通过/18skip，构建成功；初次UI测试因借用配置面板成功文案超时，71042修正断言后动态12/兼容11通过。眼验发现“已连通”还显示“尚未测试”的实际矛盾，20719新红测证实后仅修状态文案，76151最终全量同数/构建/动态13/兼容11全部通过并眼验。safeStorage、主进程、回调、DCR、三真实shim均真实运行，网络+浏览器+native确认用自有fixture适配；非供应商账号/真实TLS/模型CLI。所有进程终止，不改正式app，不发布、不广告能力。下一步真实Notion/Sentry候选与PRM/回调兼容、其余32实包，不能把纵向fixture算可用供应商。
